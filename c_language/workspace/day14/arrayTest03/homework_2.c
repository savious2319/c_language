...
/*void main()
{
	...
	while (1) {
		...
		case 3: printf("몇층 관리비를 볼까요? "); 
				scanf_s("%d", &floor); 
				printf("%d층 평균 관리비 : %d원\n", floor, floorBill(floor, arApt)); break;
		case 4: printf("본인의 층과 호수를 입력해 주세요: "); scanf_s("%d %d", &floor, &num); 
...
int row = 0;
int col = 0;
printf("원하는 아파트 층수를 입력하세요: "); scanf_s("%d", &row);
printf("원하는 아파트 호수를 입력하세요: "); scanf_s("%d", &col);
buildApt(row, col);
}
